...
export default function App() {
  ...
  return (
    <div style={{ padding: '5vw', maxWidth: 800, margin: 'auto', fontFamily: 'Arial' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <img src={logo} alt="logo" width="50" />
        <h2 style={{ color: '#003300', fontSize: '1.5rem' }}>GOZ Checker</h2>
      </div>
        ...
        {step.questions.map((q, qIndex) =>
          isVisible(q, qIndex) ? (
            <div key={qIndex} style={{
              ...
              <p style={{ marginBottom: '3vw', fontSize: '1rem' }}>
                <b>{qIndex + 1}. {q.text}</b>
                {q.hint && (
                  <button
                    type="button"
              ...
              {q.options.map((opt, idx) => (
                <label key={idx} style={{ display: 'block', marginLeft: 12, fontSize: '1rem' }}>
                  <input
                    type="radio"
